...
Computing nodes need a procedure to join to existing Alcaudon's clusters
dynamically. This feature allows scaling out when more computing resources are
...
order to detect unreachable nodes. Heartbeat arrival time are interpreted by an
...
nodes send a message to cluster coordinator in order to signal that they are
ready to receive new computations(\ref{code:noderegistering}).
...
In order to shut-down nodes gracefully, a protocol to leave Alcaudon clusters is
...
nature of distributed systems it is difficult to know if at given point in time,
all the events up to a point have been received. In the ideal world, the
...
providing the grounds to work with and reason about out of order data. That tool
...
Watermarks try to predict a $T_n$ timestamp signaling that up to that instant
...
can be used in order to predict better low watermarks. It is worth mentioning that this
...
triggered once the watermark has reached some point, i.e. all records up to a
...
actor, \lstinline[columns=fixed]{TimeExecutor}, who is in charge of keep track
and trigger timers. This actor subscribes to \acs{CRDT} watermark changes that are
...
of them has past that point, if it is the case, timer is
...
recovered once the system is restarted. Streams are implemented as actors, in
...
work pretty much as an append-only log, computations or sources publish records
...
in~\ref{code:recordstorage}. As it can bee observed, stream publishers are
...
Alcaudon deals with this problem using an hybrid approach. Stream actors store
latest message offset acknowledged by each consumer. With this information, it
...
is a configurable parameter. In order to avoid exhausted consumers, Alcaudon
...
check if they are able to process it and if they are able to handle it, take
them out of quarantine. This process is represented as a sequence diagram in
