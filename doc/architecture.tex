...
In figure~\ref{fig:statecluster} the different states in which a cluster node
...
\begin{figure}[!h]
  ...
  \caption{Compute node graceful stop protocol}
...
very same algorithm was implemented by Google\cite{milwheel} and just about $0.001\%$
...
\begin{lstlisting}[language=scala, frame=trBL, label=code:timeExecutor, float=ht, caption = {Computation state persistence}]
