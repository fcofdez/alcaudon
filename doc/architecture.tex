...
logic. To achieve this goal, Alcaudon provides certain interfaces so the users
...
As explained before, in order to create custom computations to process
...
two methods to be implemented; \lstinline[columns=fixed]{processRecord(record: Record)} and \lstinline[columns=fixed]{processTimer(timer: Timer)}. These methods
are the main entry point into user code hooked in reaction to record receipt and timers expiration.
