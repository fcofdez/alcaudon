...
cited previously. Over time, models like MapReduce\cite{mapreduce} Figure
~\ref{fig:mapreduce}, Spark\cite{spark} and Storm appeared. Those architectural
models facilitated the parallelization of data analysis processes at scale
running on commodity hardware. These systems are classified as batch processing,
meaning that all input should be collected into a batch before processing it.
...
and data analysts are becoming more demanding in their explorations. Batch
...
streaming systems, will provide approximate results in nearly real time. To
...
same principles that Google Dataflow and MillWheel are based on. It will also
provide an expressive functional API that will facilitate expressing complex
computations.
