...
object TimerExecutor {
  ...
  case class WatermarkTracker(origin: ActorRef, timer: LowWatermark, lastWatermark: Long) {
 ...
 */
class TimerExecutor(computationId: String, sources: Set[String], initialWatermark: Long = System.currentTimeMillis())
    extends Actor
    with ActorLogging {
  ...
  def updateWatermarks(watermarkTimers: List[WatermarkTracker], sourceWatermarks: Map[SourceID, Long], timestamp: Long): Unit = {
    val minWatermarkSource = Try(sourceWatermarks.values.min).getOrElse(Long.MaxValue)
    latestWatermark = oldestComputationTimestamp.min(minWatermarkSource)
    log.debug("Latest watermark {}", latestWatermark)
    val overdueTimers = watermarkTimers.filter(tracker => tracker.overdue(latestWatermark))
    val pendingTimers = watermarkTimers.filterNot(tracker => tracker.overdue(latestWatermark))
  ...
  def working(watermarkTimers: List[WatermarkTracker], sourceWatermarks: Map[SourceID, LTimestamp]): Receive = {
    ...
    case CreateTimer(timer: RecurrentFixedTimer) =>
      scheduler.schedule(timer.deadline, timer.deadline, sender(), ExecuteTimer(timer))

    case CreateTimer(timer: LowWatermark) =>
      log.debug("Creating timer with latestwatermark as {}", latestWatermark + timer.window.toMillis)
    ...
    case c @ Changed(key) =>
      ...
      log.debug("Previous source {} = new {}", sourceWatermarks, updatedSourcesWatermarks)
