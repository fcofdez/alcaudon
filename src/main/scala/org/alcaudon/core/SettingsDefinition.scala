...
class SettingsDefinition(config: com.typesafe.config.Config) {
  ...
  object blob {
    ...
    final lazy val downloadTimeout = getDuration("alcaudon.blob.download-timeout")

    object s3 {
      final lazy val accessKey = config.getString("alcaudon.blob.s3.access-key")
      final lazy val region = config.getString("alcaudon.blob.s3.region")
      final lazy val secretKey = config.getString("alcaudon.blob.s3.secret-key")
    }
  ...
  object computation {
    ...
    final lazy val distributed = config.getBoolean("alcaudon.computation.distributed")
    final lazy val maxFailures = config.getInt("alcaudon.computation.max-failures")
    final lazy val parallelism = config.getInt("alcaudon.computation.parallelism")
    final lazy val snapshotInterval = config.getInt("alcaudon.computation.snapshot-interval")
    final lazy val timeout = getDuration("alcaudon.computation.timeout")
  }

  final lazy val consistencyConstraint = config.getString("alcaudon.consistency-constraint")
  ...
  object streams {
    ...
    object flowControl {
      final lazy val backoffTime = getDuration("alcaudon.streams.flow-control.backoff-time")
      final lazy val overwhelmedDelay = config.getInt("alcaudon.streams.flow-control.overwhelmed-delay")
      final lazy val overwhelmedRetryTime = getDuration("alcaudon.streams.flow-control.overwhelmed-retry-time")
    }

    final lazy val snapshotInterval = config.getInt("alcaudon.streams.snapshot-interval")
  }
