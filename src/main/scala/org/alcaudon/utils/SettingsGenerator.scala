...
object SettingsGenerator extends App {
  ...
  val isDuration = "^(\\d+\\s*(milliseconds|ms|seconds|s|minutes|m|hours|h|days|d))$".r
  ...
  def parse(path: String, config: ConfigObject, prefix: String, string: StringBuilder): Unit = {
    for(item <- config.entrySet.asScala.toList.sortBy(_.getKey)) {
      item.getValue match {
        ...
        case s: ConfigValue =>
          ...
          if(accessor != "") {
            string.append(s"${prefix}final lazy val ${item.getKey.capitalized} = $accessor(")
